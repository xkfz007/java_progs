	...
	public String searchQuery(String queryString, String fields) {
		...
		try {
			...
			for (Document doc : qr.getRecordList()) {
				...
				// File2DocumentUtils.printDocumentInfo(doc);
				/*
				 * System.out.println("==========第" + (cnt) + "个结果================="); System.out.println("标题为：" + doc.get("title"));
				 * System.out.println("作者为：" + doc.get("author") + " 作者单位为：" + doc.get("workplace")); System.out.println("摘要为：" +
				 * doc.get("abstract"));
				 */
				result += "<p>==========第" + (cnt) + "个结果=================</p>";
				result += "<p>标题为：" + doc.get("title") + "</p>";
				// 有可能有多个作者，从中取出多个作者
				...
				for (int i = 0; i < authorNum; i++) {
					result += "<p>作者为：" + authorStr[i] + "   作者单位为：" + workplaceStr[i] + "</p>";
				}
				result += "<p>摘要为：" + doc.get("abstract") + "</p>";
				cnt++;
			}

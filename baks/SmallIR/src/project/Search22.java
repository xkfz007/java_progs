...
//						选取合适大小的摘要
		...
		//利用Termvector来提高高亮的性能
		public List<Document> getHighlightResultWithTermVector(String queryString, String field,TopDocs topDocs)
		{
...
//				 取出当前页的数据
			...
			//	int end=topDocs.totalHits;
				...
				//	System.out.println("=====" + doc.get("abstract"));
					...
					String hcStr="";
					//
					TermPositionVector termFreqVector=null;
					//
					...
					if(field.equalsIgnoreCase("title")||field.equalsIgnoreCase("abstract"))
					{
						...
						termFreqVector = (TermPositionVector)reader.getTermFreqVector(docSn, field);
						//TermFreqVector vector = reader.getTermFreqVector(docSn, field);
						tokenStream = TokenSources.getTokenStream(termFreqVector);
						 hcStr=highlighter.getBestFragment(tokenStream, doc.get(field));
						//String titleHc=highlighter.getBestFragment(analyzer, "title", doc.get("title"));
					...
					//	tokenStream = TokenSources.getTokenStream(termFreqVector);
						...
						for(int j=0;j<fieldValue.length;j++)
						{
							//hcStr=highlighter.getBestFragment(tokenStream,fieldValue[j]);
							hcStr= highlighter.getBestFragment(analyzer, field, fieldValue[j]);
					//	System.out.println("作者高亮"+authorHc);
						if(hcStr!=null)
						doc.add(new Field(field,hcStr,Store.YES, Index.ANALYZED,TermVector.WITH_POSITIONS_OFFSETS));
						else
							doc.add(new Field(field,fieldValue[j],Store.YES, Index.ANALYZED,TermVector.WITH_POSITIONS_OFFSETS));
					//选取合适大小的摘要
						String content = doc.get("abstract");
						int endIndex = Math.min(minLength, content.length());
						//

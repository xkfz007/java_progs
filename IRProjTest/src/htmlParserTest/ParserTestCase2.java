...
public class ParserTestCase2  {
	...
	//String urlStr2 = "F:\\hfz\\现代信息检索\\大作业\\测试\\papers\\P107-SIGIR2009.html";
	//String urlStr2 = "F:\\hfz\\现代信息检索\\大作业\\测试\\papers\\P107-SIGIR2009-2.html";
...
@Test
	public void testNodeFilter() {
		...
		//	parser.setURL("http://portal.acm.org/citation.cfm?doid=1571941.1571962");
...
@Test
	public void testLinkTag() {
		try {
			...
			Parser parser = new Parser();
			parser.setURL("http://www.renren.com");
			parser.setEncoding(parser.getEncoding());
			NodeList list = parser.extractAllNodesThatMatch(filter);
			for (int i = 0; i < list.size(); i++) {
...
@Test
	public void testTable() {
		...
		myParser=Parser.createParser(urlStr2, "utf-8");
				...
				if (nodeList.elementAt(i) instanceof TableTag) {
					...
					for (int j = 0; j < rows.length; j++) {
						...
						for (int k = 0; k < td.length; k++) {
							if(td[k].getAttribute("class").equalsIgnoreCase("medium-text"))
